...
실무에서 자주 사용하는 서버 구성이다.

```json
{
  "mcpServers": {
    // GitHub 연동 — PR, 이슈, 코드 검색
    "github": {
      "command": "npx",
...
### 민감한 정보는 settings.local.json에

API 키, DB URL 등은 git에 올라가면 안 된다. `settings.local.json`에 분리한다.

```json
// settings.json (git 커밋 O)
{
  "mcpServers": {
    "github": {
      "command": "npx",
...
::: warning
API 키를 `settings.json`에 직접 하드코딩하면 git에 노출된다. 반드시 환경변수로 분리하자.
:::
